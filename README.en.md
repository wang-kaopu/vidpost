...
`app/App.vue` is the production renderer entry. The production renderer uses Tailwind CSS v4 through `@tailwindcss/vite`; common controls use daisyUI 5 component classes with `corporate` as the only light theme. Layout and business states use standard Tailwind utilities, while arbitrary values are limited to dynamic sizing and domain-specific grids. Native `<dialog>` elements are centralized in `AppDialog.vue`, and the large work and account views are split into domain components. Ant Design Vue is no longer installed, and the renderer does not use `tailwind-variants` or thin control wrappers. `app/styles.css` is limited to the Tailwind/daisyUI entry points, global base rules, and required keyframes, with no `@apply`; `prettier-plugin-tailwindcss` sorts utility classes. `app/src/App.vue` and `app/src/scripts/sse-register.ts` remain the backend integration demo and must be retained.
