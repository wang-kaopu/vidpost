...
`app/App.vue` is the production renderer entry. The UI uses Tailwind CSS v4, shadcn-vue, Reka UI, and the `reka-vega` component source. Base components live in per-component directories under `app/components/ui`; business components follow the same Vue-plus-`index.ts` directory convention. Navigation uses a responsive Sidebar with a Sheet on narrow windows, while dialogs and popovers use Reka portals for focus and clipping behavior. Account and publishing-record tables use TanStack Vue Table for pagination and row selection while retaining horizontal scrolling and pinned key columns. `app/styles.css` defines a light semantic theme that keeps the existing blue primary and the `info`, `success`, `warning`, and `destructive` state meanings. Non-platform interface icons use Lucide Vue and platform logos retain their original assets. `app/src/App.vue` and `app/src/scripts/sse-register.ts` remain the backend integration demo and must be retained.
