...
`app/App.vue` is the production renderer entry. The UI uses Tailwind CSS v4 and daisyUI 5 with the `maybeghost` plugin theme. Buttons, forms, cards, tables, dialogs, drawers, notifications, and progress feedback directly compose daisyUI default components and official modifiers instead of maintaining a parallel visual component layer. Navigation uses a responsive drawer that stays open at `lg` and above and opens from a navbar under narrower viewports. Tailwind utilities are limited to layout, responsive behavior, positioning, and necessary text hierarchy; wide tables retain horizontal scrolling and positioned key columns. Non-platform interface icons use on-demand Lucide Vue imports, while platform brand logos retain their original assets. `app/styles.css` only registers Tailwind, daisyUI, and the current theme, with no additional design tokens, component overrides, or custom motion. `app/src/App.vue` and `app/src/scripts/sse-register.ts` remain the backend integration demo and must be retained.
