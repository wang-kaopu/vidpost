...
test('query errors keep status unchanged, save sync_error, and retry after 30 seconds', async () => {
  ...
  configureTaskStateServiceRuntime({
    ...
    createVideo: (() => ({ fetchPublishedState: async () => { throw new Error('network down') } })) as any,
    now: () => 0,
    onTaskChanged: (event) => events.push(event),
    resolveAccountFilePath: (() => '/tmp/account.json') as any,
    setTimeout: fake.setTimeoutFake,
    updatePublishTask: (async (_taskId, input) => { updates.push(input) }) as any,
  })
...
test('startup recovery monitors tasks with ids and fails unrecoverable history', async () => {
  ...
  configureTaskStateServiceRuntime({
    ...
    listPublishTasks: (async ({ status }) => ({
      ...
      tasks: status === 'reviewing'
        ? [reviewingWithId, reviewingWithoutId]
        : [runningWithId, runningWithoutId],
    })) as any,
    now: () => 0,
    setTimeout: fake.setTimeoutFake,
    updatePublishTask: (async (id, input) => { updates.push({ id: Number(id), input }) }) as any,
  })
  ...
  assert.equal(updates.find((item) => item.id === 6)?.input.attributes.review_state.reason, '发布记录缺少平台作品 ID，无法检测审核状态')
...
test('startup recovery backfills active Sohu record.id evidence and monitors reviewing and running tasks', async () => {
  ...
  configureTaskStateServiceRuntime({
    ...
    listPublishTasks: (async ({ status }) => ({
      ...
      lastId: 0,
      tasks: status === 'reviewing'
        ? [reviewingFromPublishResponse, reviewingFromRawId]
        : [runningFromPublishResponse],
    })) as any,
    now: () => 0,
    setTimeout: fake.setTimeoutFake,
    updatePublishTask: (async (id, input) => { updates.push({ id: Number(id), input }) }) as any,
  ...
  assert.equal(updates.find((item) => item.id === 11)?.input.attributes.review_state_clues.platform_work_id, '1049530557')
  assert.equal(updates.find((item) => item.id === 12)?.input.attributes.review_state_clues.platform_work_id, '1049530558')
  assert.equal(updates.find((item) => item.id === 13)?.input.attributes.review_state_clues.platform_work_id, '1049530559')
})
...
test('expired active Sohu history is backfilled but fails by deadline without a platform query', async () => {
  ...
  configureTaskStateServiceRuntime({
    clearTimeout: fake.clearTimeoutFake,
    createVideo: (() => ({ fetchPublishedState: async () => {
      queryCalls += 1
    ...
    setTimeout: fake.setTimeoutFake,
    updatePublishTask: (async (_id, input) => { updates.push(input) }) as any,
  })
