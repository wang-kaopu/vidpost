...
async function createSohuAccountFile(): Promise<string> {
  const directory = await temporaryDirectories.create("sohu-state-");
...
test("搜狐准确映射全部官方 record.status 状态值", () => {
  const cases = [
...
test("搜狐仅对未公开状态使用 rejectReason", () => {
  assert.equal(parseSohuRecordStatus({ id: 1, status: 3, rejectReason: "  封面违规  " }).reason, "封面违规");
  assert.equal(
    parseSohuRecordStatus({ id: 1, status: 4, rejectReason: "历史残留原因" }).reason,
    "已发布（搜狐状态码 4）",
  );
});

test("搜狐拒绝未知、字符串及格式错误的状态记录", () => {
...
test("搜狐记录收集器接受 data.news 数组和 data.videos 数字键对象", () => {
...
test("搜狐匹配器仅匹配 record.id，不匹配 clientNewsId 或标题", () => {
  const records = [
...
test("搜狐 HTTP 查询发送必要请求头并跨三页传递 streamId", async () => {
...
test("搜狐遇到空页后停止查询并将缺失作品标记为未公开", async () => {
  const accountFile = await createSohuAccountFile();
  const previousAdapter = axios.defaults.adapter;
  let calls = 0;
  axios.defaults.adapter = async (config) => {
...
test("搜狐在三个非空页面均找不到作品 ID 时标记为未公开", async () => {
  const accountFile = await createSohuAccountFile();
  const previousAdapter = axios.defaults.adapter;
  axios.defaults.adapter = async (config) => ({
    config,
...
test("搜狐报告登录及其他业务错误且不在 Axios 内重试", async () => {
  const accountFile = await createSohuAccountFile();
  const previousAdapter = axios.defaults.adapter;
  const responses = [
...
test("搜狐传递网络和响应结构错误且不内部重试", async () => {
  const accountFile = await createSohuAccountFile();
  const previousAdapter = axios.defaults.adapter;
  let calls = 0;
  ...
  try {
    await assert.rejects(
      new SohuVideo().fetchPublishedState({
        accountFile,
