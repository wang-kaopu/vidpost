...
const { buildElectronLaunchConfig } = require('../scripts/start-electron.cjs')
...
test('start-electron clears ELECTRON_RUN_AS_NODE before launch', () => {
  ...
  assert.deepEqual(args, [path.resolve(__dirname, '..'), '--inspect'])
  assert.equal(spawnOptions.cwd, path.resolve(__dirname, '..'))
