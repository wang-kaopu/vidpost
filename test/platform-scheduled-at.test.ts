...
test("platform publish payloads map scheduledAt to their protocol fields", () => {
  ...
  const bilibiliSource = fs.readFileSync(path.join(videoDirectory, "bilibili", "publish.ts"), "utf8");
  const baijiahaoSource = fs.readFileSync(path.join(videoDirectory, "baijiahao", "publish.ts"), "utf8");
  const douyinSource = fs.readFileSync(path.join(videoDirectory, "douyin", "upload.ts"), "utf8");
