...
import type { PublishedStateResult, VideoPlatformType } from '../infra/video/video.ts'
...
function buildFetchPayload(task: PublishTask, abortSignal?: AbortSignal) {
    const attributes = normalizeRecord(task.attributes)
    const platform = normalizeString(task.platform)
    const accountFile = task.accountId && platform
        ? runtime.resolveAccountFilePath(task.accountId, platform)
        : null
    return {
        abortSignal,
        accountFile,
...
async function persistTerminalState(monitor: TaskMonitor): Promise<boolean> {
    ...
    try {
        await runtime.updatePublishTask(monitor.task.id, {
            status: terminal.status,
            link: terminal.link,
            attributes,
        })
...
async function runTaskMonitor(monitor: TaskMonitor): Promise<void> {
    ...
    try {
        const fetchResult = await runtime.createVideo(platform).fetchPublishedState(
            buildFetchPayload(monitor.task, monitor.abortController.signal),
        )
...
export function startTaskStateMonitor(task: PublishTask): boolean {
    ...
    if (!Number.isInteger(taskId) || !platform || !MONITORED_PLATFORMS.has(platform) || !getPlatformWorkId(task)) return false
...
async function listAllTasksByStatus(status: string, limit = DEFAULT_LIST_LIMIT): Promise<PublishTask[]> {
    ...
    while (true) {
        ...
        if (response.isEnd || pageTasks.length === 0 || !Number.isInteger(response.lastId) || response.lastId <= lastId) break
...
export async function recoverTaskStateMonitors(options: { limit?: number } = {}): Promise<void> {
    const limit = Number.isInteger(options.limit) && Number(options.limit) > 0 ? Number(options.limit) : DEFAULT_LIST_LIMIT
    ...
    for (const originalTask of [...reviewingTasks, ...runningTasks]) {
        ...
        const reason = task.status === RUNNING_STATUS
            ? '发布过程被中断，请重试'
