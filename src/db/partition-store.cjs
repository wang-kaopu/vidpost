...
const PARTITION_MAP_TABLE_KEY = 'partition_map_table'
 ...
 */
function movePartitionMapping(store, fromAccountId, toAccountId) {
 ...
 */
function deletePartitionMapping(store, accountId) {
  const normalizedAccountId = String(accountId || '').trim()
  if (!normalizedAccountId) {
    return
  }
...
module.exports = {
  ...
  encodePartitionAccountId,
  movePartitionMapping,
  readPartitionMapTable,
  resolveDefaultPartitionStorePath,
