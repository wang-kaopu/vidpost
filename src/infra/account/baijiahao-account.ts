...
var waitForWebContentsIdle = async (webContents, idleMs, timeoutMs) => {
...
function isBaijiahaoLoginSuccessUrl(url) {
  try {
...
var BAIJIAHAO_LOGIN_URL = "https://baijiahao.baidu.com/builder/theme/bjh/login";
var BAIJIAHAO_CLOSE_BUTTON_SCRIPT = buildCloseButtonScript(
  "matrix-baijiahao-login-close",
  "matrix-baijiahao-login"
);

var runBaijiahaoLogin = async (options) => runPlatformLoginFlow(
  {
    title: "\u767E\u5BB6\u53F7\u767B\u5F55",
    ...
    beforePersist: async (loginWindow) => {
      await waitForWebContentsIdle(loginWindow.webContents, 1500, 1e4);
    }
  },
  options
);
