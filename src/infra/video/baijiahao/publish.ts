...
interface BaijiahaoRunContext {
  appId: string;
  cookieHeader: string;
  fileMd5: string;
  fileModifiedAt: number;
  metadata: BaijiahaoVideoMetadata;
...
export async function prepareBaijiahaoPublish(input: BaijiahaoVideoUploadPayload): Promise<BaijiahaoPreparedContext> {
  ...
  const [cookieHeader, metadata, videoStats, fileMd5] = await Promise.all([
    ...
    inspectBaijiahaoMp4(videoPath),
    stat(videoPath),
    calculateBaijiahaoVideoMd5(videoPath),
  ]);
  ...
  const covers = await generateBaijiahaoCovers(coverPath);
...
/** 发送百家号最后一次发布请求。 */
export async function commitBaijiahaoPublish(prepared: BaijiahaoPreparedContext): Promise<VideoUploadResult> {
