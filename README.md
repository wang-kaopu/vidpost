...
`app/App.vue` 是正式渲染入口；界面使用 Tailwind CSS v4、shadcn-vue、Reka UI 和 `reka-vega` 组件源码。基础组件按目录维护在 `app/components/ui`，业务组件也采用目录、Vue 实现和 `index.ts` 出口的结构。应用导航使用响应式 Sidebar，窄窗口由 Sheet 承载；弹窗、菜单和选择浮层通过 Reka Portal 管理焦点与裁剪边界。账号和发布记录表格使用 TanStack Vue Table 管理分页与选择状态，并继续保留横向滚动和关键列定位。`app/styles.css` 维护浅色语义 token，保留原主题的蓝色主色以及 `info`、`success`、`warning`、`destructive` 状态含义。非平台界面图标使用 Lucide Vue，平台品牌 Logo 保留原始资源。`app/src/App.vue` 和 `app/src/scripts/sse-register.ts` 是后端联调 Demo，不在正式界面迁移范围内，也不能作为废弃目录删除。
