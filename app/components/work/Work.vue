...
<template>
  <section class="min-w-0">
    ...
    <div
      ...
      class="flex min-h-48 items-center justify-center gap-3 text-muted-foreground"
    >
    ...
    >
      ...
      <Button type="button" size="sm" variant="outline" @click="reloadWorks">重新加载</Button>
    </Alert>

    <div
      v-else-if="!appConfig.isMockMode && worksList.length > 0"
      ref="sentinelRef"
      class="flex min-h-16 items-center justify-center px-6 text-sm text-muted-foreground"
    >
      <Spinner v-if="loadingMore" />
      <span v-else-if="loadError" class="text-destructive">{{ loadError }}</span>
      <span v-else-if="isEnd">没有更多了</span>
      <span v-else>下拉加载更多</span>
    </div>

    <Empty v-if="!loading && !loadError && worksList.length === 0" class="mx-6 min-h-60 border-0">
      <EmptyHeader
        ><EmptyTitle>暂无作品</EmptyTitle><EmptyDescription>符合条件的作品会显示在这里</EmptyDescription></EmptyHeader
      >
    </Empty>
  </section>
