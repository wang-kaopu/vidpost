...
export type PublishProgressTask = PublishProgressTaskInput & {
...
export function createPublishProgressCenter(): PublishProgressCenterApi {
  ...
  const complete = (taskId: string): void => {
    items.value = items.value.map((item) => item.id === taskId
      ? { ...item, phase: item.scheduled ? "scheduled" : "completed", errorMessage: "" }
      : item);
  };
